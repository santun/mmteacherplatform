...
@section('title', __('Resource'). ' ['. (isset($post->id) ? 'Edit #'.$post->id : 'New '. title_case($format)).']')
        ...
        <div class="container text-center h-50">
            <div class="row">
                <div class="col-md-8 mx-auto">
                   <!--<h1>{{ __('Resource'). ' ['. (isset($post->id) ? 'Edit #'.$post->id : 'New '. title_case($format)).']' }}</h1>-->
                </div>
            </div>
        </div>
    </header>
        ...
        <div class="container">
            <div class="row gap-y">
                ...
                <div class="col-md-9 mx-auto" id="elibrary_root">
                    <h4>
                        {{ __('Resource') }}
                        @if (isset($post->id)) [Edit] @else [New {{ title_case($format) }}] @endif
                    </h4>

                        @if (isset($post))
                            {!! \Form::open(array('files' => true, 'method' => 'put', 'route' => array('member.resource.update', $post->id)
                            , 'class' => 'form-horizontal', '@submit' => 'validateBeforeSubmit')) !!}
                        @else
                            {!! \Form::open(array('files' => true, 'route' => 'member.resource.store',
                            'class' => 'form-horizontal', '@submit' => 'validateBeforeSubmit')) !!}
                        @endif
                    ...
                    @if (isset($post->id) && $post->published != 1 && $post->approval_status === null)
                    <div class="alert alert-info" role="alert">
                        <h4 class="alert-heading">{{ __('Well done!') }}</h4>
                       <div>{{ __('If your resource is ready to appear in our eLibrary, you can submit a request for approval.') }}</div>
                      <div class="mt-5 mb-0">
                            <a class="btn btn-info" href="{{ route('member.resource.submit-request', $post->id) }}">
                            {{ __('Submit') }}
                        </a>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="form-group">
                                <label for="resource_format" class="col-xs-12 require">{{__('Resource Format') }}</label>
                                {!! Form::select('resource_format',
                                $formats, old('resource_format', isset($post->resource_format)? $post->resource_format: ''), ['class' => $errors->has('resource_format')?
                            ...
                            <div class="form-group">
                                <label for="strand" class="require">{{ __('Strand') }}
                                    <i class="fa fa-info-circle" data-provide="tooltip" data-toggle="tooltip"
                                       data-placement="top"
                                       data-original-title="Strand"></i>
                                </label>
                            ...
                            <div class="form-group">
                                <label for="sub_strand" class="require">{{ __('Sub Strand') }}
                                    <i class="fa fa-info-circle" data-provide="tooltip" data-toggle="tooltip"
                                       data-placement="top"
                                       data-original-title="Sub Strand"></i>
                                </label>
                            ...
                            <div class="form-group">
                                <label for="lesson" class="require">{{ __('Lesson') }}
                                    <i class="fa fa-info-circle" data-provide="tooltip" data-toggle="tooltip"
                                       data-placement="top"
                                       data-original-title="Lesson"></i>
                                </label>
                            ...
                            <div class="form-group">
                                <label for="author" class="col-xs-12 require">{{ __('Author') }}</label>
                            ...
                            <div class="form-group">
                                <label for="publisher" class="col-xs-12">{{ __('Publisher') }}</label>
                            ...
                            <div class="form-group">
                                <label for="suitable_for_ec_year" class="col-xs-12 require">{{ __('Suitable for Education College Year(s)') }}</label>
                                ...
                                @foreach ($years as $key => $year)
                                    ...
                                    {{ Form::checkbox('suitable_for_ec_year[]', $key, in_array($key, $ec_years) ? true : false, ['id' => 'ecy_' . $key, 'class' => $errors->has('suitable_for_ec_year')? 'form-check-input is-invalid' : 'form-check-input', 'v-validate' => "'required'"] ) }}
                                    @else
                                    {{ Form::checkbox('suitable_for_ec_year[]', $key, '', ['id' => 'ecy_' . $key, 'class' => $errors->has('suitable_for_ec_year')? 'form-check-input is-invalid' : 'form-check-input', 'v-validate' => "'required'"]) }}
                                    @endif
                            ...
                            <div class="form-group">
                                ...
                                {!! Form::select('keywords[]', $selectedKeywords, $selectedKeywords, [ 'id' => 'keyword-list', 'multiple' => 'true', 'class' => 'form-control']) !!}
                                {!! $errors->first('keywords', '<div class="invalid-feedback">:message</div>') !!}
                            </div>

{{--                             <div class="form-group">
                                <label for="url" class="col-xs-12">{{ __('Keywords by other') }}</label>

                                {!! Form::select('keywords_other[]', $selectedOtherKeywords, $selectedOtherKeywords, [ 'id' => 'other-keyword-list', 'multiple' => 'true', 'class' => 'form-control']) !!}
                                {!! $errors->first('keywords_other', '<div class="invalid-feedback">:message</div>') !!}
                            </div> --}}

                            <div class="form-group">
                                <label for="url" class="col-xs-12">{{ __('URL') }}</label>
                            ...
                            <div class="form-group">
                                <label for="additional_information">{{ __('Additional Information') }}
                                    <i class="fa fa-info-circle" data-provide="tooltip" data-toggle="tooltip"
                                       data-placement="top"
                                       data-original-title="Adding extra information of the resource"></i>
                                </label>
                                <input type="text" placeholder="Additional Information.." name="additional_information" id="additional_information"
                            ...
                            <div class="form-group">
                                ...
                                <textarea v-validate="'required'" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}"
                                    rows="5" name="description" id="description">{{ old('description', isset($post->description) ? $post->description: '') }}</textarea>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="subject_id" class="require">{{ __('Subjects') }}</label>
                                @foreach ($subjects as $subject)
                                    <div class="form-check">
                                        @if (isset($post) && $post->subjects)
                                         ...
                                         {{ Form::checkbox('subjects[]', $subject->id, in_array($subject->id, $subjects)? true : false, ['id' => 'sub_' . $subject->id,
                                        ...
                                        <label class="form-check-label"> {{ Form::label('sub_' . $subject->id, $subject->title) }} </label>
                                        @if ($loop->last)
                                ...
                                <div class="col-xs-12">
                                    ...
                                    <div>
                                        @foreach($images as $image)
                                            ...
                                            <a class="text-danger" onclick="return confirm('Are you sure you want to delete?')" href="{{ route('member.media.destroy', $image->id) }}">{{ __('Remove') }}</a>
                                        @endforeach
                                    </div>
                                    @endif
                                ...
                                <div class="col-xs-12">
                                    ...
                                    @if($image->getCustomProperty('uri'))
                                        <a target="_blank" href="{{ $vimeo_url ? $vimeo_url : asset($image->getUrl()) }}">
                                            <img src="{{ ($image->getCustomProperty('uri'))? $vimeo_thumbnail : asset($image->getUrl()) }}" style="border:1px solid #ccc; margin-right: 10px">
                                        </a>
                                    @else
                            ...
                            <div class="form-group">
                                <div class="col-xs-12">
                                    ...
                                    {{ Form::file('full_version_files', ['class' => $errors->has('full_version_files') ? 'form-control is-invalid' : '']) }}
                            ...
                            <div class="form-group">
                                ...
                                $approvalStatus, old('approval_status', isset($post->approval_status) ? $post->approval_status: ''), ['placeholder' => '-Approval Status-', 'class' => $errors->has('approval_status')?
                                'form-control is-invalid' : 'form-control' ]) !!} {!! $errors->first('approval_status', '
                                <div class="invalid-feedback">:message</div>') !!}
                            </div>
                            @endif

                            @if (isset($post) && !is_null($post->id))
                            <div class="form-group">
                                <div>{{ __('Created By') }}: {{ $post->user->name ?? '' }}</div>
                                <div>{{ __('Created At') }}: {{ $post->user->created_at ?? '' }}</div>
                                <div>{{ __('Last Updated') }}: {{ $post->user->updated_at ?? '' }}</div>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <button class="btn btn-primary" type="submit" name="btnNext" value="1">
                            {{ __('Next') }}
                        </button>
