...
<div class="card">
    <header class="card-header">
        ...
        <h4 class="card-title">
            @can('add_license')
            <a href="{{ route('admin.license.create') }}" class="btn btn-primary text-white">New</a>
            @endcan
        </h4>

        <form action="{{ route('admin.license.index') }}" method="get">
            <div class="card-header-actions">
                ...
                <a href="{{ route('admin.license.index') }}" class="btn">{{ __('Reset') }}</a>
            </div>
        </form>
    </header>
                ...
                <tr>
                    ...
                    <td class="text-right table-options">
                        ...
                        <a class="table-action hover-primary cat-edit"
                            href="{{ route('admin.license.edit', $post->id) }}" data-provide="tooltip" title="Edit"><i
                                class="ti-pencil"></i></a>
                        @endcan
                        @can('delete_license')
                        {!! Form::open(array('route' => array('admin.license.destroy', $post->id), 'method' => 'delete'
