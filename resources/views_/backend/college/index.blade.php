...
@section('content')`
    <div class="card">
        <header class="card-header">
            <h4 class="card-title">
                @can('add_college')
                    <a href="{{ route('admin.college.create') }}" class="btn btn-primary text-white">{{ __('New') }}</a>
                @endcan
            </h4>
            <form action="{{ route('admin.college.index') }}" method="get">
                <div class="card-header-actions">
                    ...
                    <a href="{{ route('admin.college.index') }}" class="btn">{{ __('Reset') }}</a>
                </div>
            </form>
        </header>
                ...
                @foreach($posts as $post)
                    <tr>
                        ...
                        <td class="text-right table-options">
                            ...
                            @can('delete_college')
                                {!! Form::open(array('route' => array('admin.college.destroy', $post->id), 'method' => 'delete' , 'onsubmit'	=> 'return confirm("Are you sure you want to delete?");', 'style' => 'display: inline', '')) !!}
